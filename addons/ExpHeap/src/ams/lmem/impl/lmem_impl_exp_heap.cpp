...
#include "ams/lmem/impl/lmem_impl_common_heap.hpp"
#include <cstdlib>
...
namespace ams::lmem::impl {
    ...
    namespace {
        ...
        HeapHead* InitializeExpHeap(void* start, void* end, u32 option) {
            ...
            {
                MemoryRegion region {
                    .start = heap_head->heap_start,
                    .end = heap_head->heap_end,
                };
        ...
        void* ConvertFreeBlockToUsedBlock(ExpHeapHead* head, ExpHeapMemoryBlockHead* block_head, void* block, size_t size, AllocationDirection direction) {
            ...
            MemoryRegion free_region_back {
                .start = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(block) + size),
                .end = free_region_front.end,
            };
            ...
            if ((GetPointerDifference(free_region_front.start, free_region_front.end) < sizeof(ExpHeapMemoryBlockHead) + MinimumFreeBlockSize) || (direction == AllocationDirection_Front && !head->use_alignment_margins && GetPointerDifference(free_region_front.start, free_region_front.end) < MaximumPaddingalignment)) {
            ...
            if ((GetPointerDifference(free_region_back.start, free_region_back.end) < sizeof(ExpHeapMemoryBlockHead) + MinimumFreeBlockSize) || (direction == AllocationDirection_Back && !head->use_alignment_margins && GetPointerDifference(free_region_back.start, free_region_back.end) < MaximumPaddingalignment)) {
                /* There isn't enough space for a new block, or else we're not allowed to make one. */
        ...
        void* AllocateFromHead(HeapHead* heap, size_t size, s32 alignment) {
            ...
            for (auto it = exp_heap_head->free_list.begin(); it != exp_heap_head->free_list.end(); it++) {
                ...
                const uintptr_t block_start = hk::alignUp(absolute_block_start, alignment);
    ...
    HeapHandle CreateExpHeap(void* address, size_t size, u32 option) {
        ...
        const uintptr_t uptr_start = hk::alignUp(reinterpret_cast<uintptr_t>(address), MinimumAlignment);
    ...
    void* AllocateFromExpHeap(HeapHandle handle, size_t size, s32 alignment) {
        ...
        HK_ASSERT((abs_alignment & (abs_alignment - 1)) == 0);
        HK_ASSERT(MinimumAlignment <= static_cast<size_t>(abs_alignment));

        /* Fix size to be correctly aligned. */
        if (size == 0) {
            size = 1;
        }
        size = hk::alignUp(size, MinimumAlignment);
    ...
    void FreeToExpHeap(HeapHandle handle, void* mem_block) {
        ...
        HK_ASSERT(IsValidHeapHandle(handle));
        HK_ASSERT(IsValidUsedMemoryBlock(handle, mem_block));
    ...
    size_t GetExpHeapMemoryBlockSize(const void* memory_block) {
        HK_ASSERT(IsValidUsedMemoryBlock(nullptr, memory_block));
