...
namespace nvn {
    ...
    struct TextureDepthStencilMode {
        enum Enum { DEPTH,
            STENCIL,
            NVN_TEXTURE_DEPTH_STENCIL_MODE_LARGE = -1 };
    ...
    struct QueueAcquireTextureResult {
        enum Enum { SUCCESS,
            NATIVE_ERROR,
            NVN_QUEUE_ACQUIRE_TEXTURE_RESULT_LARGE = -1 };
    ...
    struct WindowAcquireTextureResult {
        enum Enum { SUCCESS,
            NATIVE_ERROR,
            NVN_WINDOW_ACQUIRE_TEXTURE_RESULT_LARGE = -1 };
    ...
    struct SyncFlagBits {
        enum Enum { FLUSH_FOR_CPU = 1,
            NVN_SYNC_FLAG_BITS_LARGE = -1 };
    ...
    struct DebugCallbackSource {
        enum Enum { API,
            NVN_DEBUG_CALLBACK_SOURCE_LARGE = -1 };
    ...
    struct DebugCallbackType {
        enum Enum { API_ERROR,
            API_WARNING,
            NVN_DEBUG_CALLBACK_TYPE_LARGE = -1 };
    ...
    struct WindowOriginMode {
        enum Enum { LOWER_LEFT,
            UPPER_LEFT,
            NVN_WINDOW_ORIGIN_MODE_LARGE = -1 };

        NVN_ENUM(WindowOriginMode);
    };

    struct DepthMode {
        enum Enum { NEAR_IS_MINUS_W,
            NEAR_IS_ZERO,
            NVN_DEPTH_MODE_LARGE = -1 };
    ...
    struct CoverageModulationMode {
        enum Enum { NONE,
            ...
            ALPHA,
            RGBA,
            NVN_COVERAGE_MODULATION_MODE_LARGE = -1 };

        NVN_ENUM(CoverageModulationMode);
    };

    struct QueueFlags {
        enum Enum { NO_FRAGMENT_INTERLOCK = 1,
            NO_ZCULL = 2,
            NONE = 0,
    ...
    struct EventSignalLocation {
        enum Enum { TOP,
            VERTEX_PIPE,
            BOTTOM,
            NVN_EVENT_SIGNAL_LOCATION_LARGE = -1 };

        NVN_ENUM(EventSignalLocation);
    };

    struct EventSignalFlags {
        enum Enum { NO_WAIT_MEMORY = 1,
            NVN_EVENT_SIGNAL_FLAGS_LARGE = -1 };
