...
namespace hk::fsp {
    ...
    public:
        ...
        Result createFile(std::string_view path, s64 size, u32 flags) {
        ...
        Result deleteFile(std::string_view path) {
            ASSERT_PATH_LEN(path);
        ...
        Result createDirectory(std::string_view path) {
            ASSERT_PATH_LEN(path);
        ...
        Result deleteDirectory(std::string_view path) {
            ASSERT_PATH_LEN(path);
        ...
        Result deleteDirectoryRecursive(std::string_view path) {
            ASSERT_PATH_LEN(path);
        ...
        Result renameFile(std::string_view oldPath, std::string_view newPath) {
        ...
        Result renameDirectory(std::string_view oldPath, std::string_view newPath) {
            ASSERT_PATH_LEN(oldPath);
            ASSERT_PATH_LEN(newPath);
        ...
        ValueOrResult<IFile> openFile(std::string_view path, FileMode mode) {
            ASSERT_PATH_LEN(path);
        ...
        ValueOrResult<sf::Service> openDirectory(std::string_view path, DirectoryFilter filter) {
            ASSERT_PATH_LEN(path);
